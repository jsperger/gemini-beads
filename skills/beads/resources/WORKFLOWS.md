...
- [Integration with Session Todos (write_todos)](#integration-with-todowrite) - Using both tools together
...
- [ ] Reconstruct Session Todos (write_todos) list from notes if needed
```
...
### At Session Start (Gemini's responsibility)
...
### At Session End (Gemini prompts user)
...
```
User Tips:
- [ ] When stopping work: Let Gemini suggest notes update
- [ ] When resuming: Let Gemini read notes and report context
...
# Gemini reports: "Markdown→Docs feature is 80% parsed. We were testing
...
## Integration with Session Todos (write_todos)
...
- [ ] Create Session Todos (write_todos) from acceptance criteria for execution
- [ ] Work through Session Todos (write_todos) items
- [ ] Update bd design notes as you learn
- [ ] When Session Todos (write_todos) complete, close bd issue
```

**Why hybrid**: bd provides persistent structure, Session Todos (write_todos) provides visible progress.
...
**Should I create a bd issue or use Session Todos (write_todos)?**
...
1. Transition from Session Todos (write_todos) to bd mid-session
