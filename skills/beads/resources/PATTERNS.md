...
2. Create Session Todos (write_todos) for immediate work:
   ```
   - [ ] Draft cost-benefit analysis
   - [ ] Ask user about budget constraints
   - [ ] Finalize recommendations
   ```
3. Work on tasks, mark Session Todos (write_todos) items completed
...
**Outcome**: Session Todos (write_todos) disappears at session end, but bd notes preserve context for next session.

**Key insight**: Notes field captures the "why" and context, Session Todos (write_todos) tracks the "doing" right now.
...
6. **Begin work**: Create Session Todos (write_todos) from notes field's NEXT section
...
# Now create Session Todos (write_todos) based on NEXT section
```
...
- Session Todos (write_todos) lists
- Recent discussion
...
3. **Reconstruct Session Todos (write_todos) from notes**:
   - COMPLETED section: Done, skip
   - IN PROGRESS section: Current state
   - NEXT section: **This becomes your Session Todos (write_todos) list**
...
2. Create Session Todos (write_todos):
   ```
