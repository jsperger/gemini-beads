# Boundaries: When to Use bd vs Session Todos (write_todos)

This reference provides detailed decision criteria for choosing between bd issue tracking and Session Todos (write_todos) for task management.
...
- [Decision Matrix](#decision-matrix)
  ...
  - [Use Session Todos (write_todos) for](#use-todowrite-for): Single-Session Tasks, Linear Execution, Immediate Context, Simple Tracking
- [Detailed Comparison](#detailed-comparison)
- [Integration Patterns](#integration-patterns)
  - Pattern 1: bd as Strategic, Session Todos (write_todos) as Tactical
  - Pattern 2: Session Todos (write_todos) as Working Copy of bd
...
- [Common Mistakes](#common-mistakes)
  - Using Session Todos (write_todos) for multi-session work, using bd for simple tasks, not transitioning when complexity emerges, creating too many bd issues, never using bd
...
- If markdown skim would suffice → **Session Todos (write_todos) is fine**

This heuristic captures the essential difference: bd provides structured context that persists across long gaps, while Session Todos (write_todos) excels at immediate session tracking.
...
### Use Session Todos (write_todos) for:
...
**Why Session Todos (write_todos) wins**: Simple checklist is perfect for linear execution. No need for persistence or dependencies. Clear completion within session.
...
**Why Session Todos (write_todos) wins**: Steps are predetermined and sequential. No discovery, no blockers, no side quests. Just execute top to bottom.
...
**Why Session Todos (write_todos) wins**: No external context to track. Everything needed is in current conversation. Session Todos (write_todos) provides user visibility, nothing more needed.
...
**Why Session Todos (write_todos) wins**: User wants to see thinking and progress. Session Todos (write_todos) is visible in conversation. bd is invisible background structure.

---

## Detailed Comparison

| Aspect | bd | Session Todos (write_todos) |
|--------|-----|-----------|
...
bd and Session Todos (write_todos) can coexist effectively in a session. Use both strategically.

### Pattern 1: bd as Strategic, Session Todos (write_todos) as Tactical

**Setup:**
- bd tracks high-level issues and dependencies
- Session Todos (write_todos) tracks current session's execution steps
...
Session Todos (write_todos) (for JWT validation):
- [ ] Install JWT library
...
- Create Session Todos (write_todos) checklist from bd issue's acceptance criteria
- Update bd as Session Todos (write_todos) items complete

**Example:**
```
Session start:
- Check bd: "issue-auth-42: Add JWT token validation" is ready
- Extract acceptance criteria into Session Todos (write_todos)
- Mark bd issue as in_progress
- Work through Session Todos (write_todos) items
- Update bd design notes as you learn
- When Session Todos (write_todos) completes, close bd issue
```
...
**From Session Todos (write_todos) to bd:**
...
**How to transition:**
```
1. Create bd issue with current Session Todos (write_todos) content
2. Note: "Discovered this is multi-session work during implementation"
3. Add dependencies as discovered
4. Keep Session Todos (write_todos) for current session
5. Update bd issue before session ends
6. Next session: resume from bd, create new Session Todos (write_todos) if needed
```

**From bd to Session Todos (write_todos):**
...
2. Create Session Todos (write_todos) from issue description
3. Execute via Session Todos (write_todos)
...
**Session Todos (write_todos) role**: None initially. Might use Session Todos (write_todos) for single-session testing sprints once migration scripts ready.

### Example 2: Simple Feature Implementation

**Scenario**: Add logging to existing endpoint based on clear specification.

**Why Session Todos (write_todos)**:
- Single session work
...
**Initial assessment**: Seems simple, try Session Todos (write_todos) first.
...
**Session Todos (write_todos) role**: Could use Session Todos (write_todos) for individual controller updates as implementing.
...
### Mistake 1: Using Session Todos (write_todos) for Multi-Session Work
...
**Solution**: Use Session Todos (write_todos). It's designed for exactly this case.

### Mistake 3: Not Transitioning When Complexity Emerges

**What happens**:
- Start with Session Todos (write_todos) for "simple" task
- Discover blockers and dependencies mid-way
- Keep using Session Todos (write_todos) despite poor fit
...
### Mistake 5: Never Using bd Because Session Todos (write_todos) is Familiar
...
**"This looks straightforward"** → Session Todos (write_todos)

**As work progresses:**

✅ **Stays straightforward** → Continue with Session Todos (write_todos), complete in session
...
**Time horizon:**
- Same session → Session Todos (write_todos)
- Multiple sessions → bd

**Dependency structure:**
- Linear steps → Session Todos (write_todos)
- Blockers/prerequisites → bd

**Scope clarity:**
- Well-defined → Session Todos (write_todos)
- Exploratory → bd

**Context complexity:**
- Conversation has everything → Session Todos (write_todos)
- External context needed → bd

**User interaction:**
- User watching progress → Session Todos (write_todos) visible in chat
- Background work → bd invisible structure

**Resume difficulty:**
- Easy from markdown → Session Todos (write_todos)
