# Beads Skill for Gemini Code

A comprehensive skill for using [beads](https://github.com/steveyegge/beads) (bd) issue tracking with Gemini Code.

## What This Skill Does

This skill teaches Gemini Code how to use bd effectively for:
...
Copy the `beads/` directory to your Gemini Code skills location:
...
- Tasks too complex for simple Session Todos (write_todos) lists

## File Structure

```
beads/
├── SKILL.md                 # Main skill file (Gemini reads this first)
...
└── resources/               # Detailed documentation (loaded on demand)
    ├── BOUNDARIES.md        # When to use bd vs Session Todos (write_todos)
    ├── CLI_REFERENCE.md     # CLI command reference
    ├── DEPENDENCIES.md      # Dependency semantics (A blocks B vs B blocks A)
    ├── INTEGRATION_PATTERNS.md # Session Todos (write_todos) and other tool integration
...
### bd vs Session Todos (write_todos)

| Use bd when... | Use Session Todos (write_todos) when... |
|----------------|----------------------|
...
When Gemini's context gets compacted, conversation history is lost but bd state survives. Write notes as if explaining to a future Gemini with zero context:
